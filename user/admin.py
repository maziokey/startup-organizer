...
class UserAdmin(admin.ModelAdmin):
    # list view
    actions = ['make_staff']
    list_display = ('username', 'get_name', 'email', 'get_date_joined', 'is_staff', 'is_superuser')
    ...
    def user_change_password(self, request, user_id, form_url=''):
        ...
        if user is None:
            raise Http404(
                ...
                '{key} does not exist.'.format(name=force_text(self.model._meta.verbose_name), key=escape(user_id))
            )
        ...
        context = {
            'title': 'Change password: {}'.format(escape(user.get_username())),
            'form_url': form_url,
            'form': form,
